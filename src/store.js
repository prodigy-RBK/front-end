...
export default new Vuex.Store({
  ...
  actions: {
    ...
    UPDATE_USER_WISHLIST: async function(state) {
      let { data } = await axios.get("http://localhost:3000/api/user/wishlist");
      this.commit("UPDATE_WISHLIST", data.wishlist);
    },
    ADD_TO_WISHLIST: async function(state, payload) {
      let { data } = await axios.put("http://localhost:3000/api/user/wishlist", { product: payload });
      this.commit("UPDATE_WISHLIST", data);
    },
    REMOVE_FROM_WISHLIST: async function(state, payload) {
      let { data } = await axios.delete("http://localhost:3000/api/user/wishlist", {
        data: {
          product: payload
        }
      });
